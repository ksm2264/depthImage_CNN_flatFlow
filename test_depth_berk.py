...
#testList = glob.glob('/media/karl/DATA/retinalCNN_data/JAW_*/*.mat')
...
for subj in ['s3','s4','s5','s6','s7','s8','s9','s10']:
    
    testList = glob.glob('/media/karl/DATA/retinalCNN_data/{subj}_*/*.mat'.format(subj=subj))
    ...
    for ee in range(30):
        if os.path.exists('results/depth/{ee}.npy'.format(ee=ee)):
            model = tf.keras.models.load_model('models_depth/{ee}'.format(ee=ee))
            ...
            for idx,pred_map in enumerate(list(test)):
                ...
                this_map = cv2.applyColorMap(np.round((pred_map/np.max(pred_map))*255).astype(np.uint8),cv2.COLORMAP_JET)
...
#                
#                cv2.imwrite(this_image_str,out_img)
#                overlayed_img.append(out_img)
        ...
        #    this_medians.append(np.median(quants))
            ...
            print(ee)
            if not os.path.exists('results/depth/{subj}'.format(subj=subj)):
                
                os.mkdir('results/depth/{subj}'.format(subj=subj))
            np.save('results/depth/{subj}/{ee}.npy'.format(subj=subj,ee=ee),{'mean':np.nanmean(quants),'median':np.nanmedian(quants)})
    #model.save('63')
