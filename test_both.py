...
testList = glob.glob('/media/karl/DATA/retinalCNN_data/JAC_1/*.mat')
...
def getInputStack_saliency_rgb_flow(fileList):
    

    x = np.stack([my_readfunction_flow(filename) for filename in fileList])
    y = np.stack([my_readfunction_out(filename) for filename in fileList])
    z = np.stack([my_readfunction_rgb(filename) for filename in fileList])
...
def getInputStack_saliency_rgb_mag_diff(fileList):
    

    x = np.stack([my_readfunction_mag_diff(filename) for filename in fileList])
    y = np.stack([my_readfunction_out(filename) for filename in fileList])
    z = np.stack([my_readfunction_rgb(filename) for filename in fileList])
...
for mode in ['flow','mag_diff']:
    ...
    if mode=='flow':
        x_test,y_test,rgb_test,foot_ii_jj = getInputStack_saliency_rgb_flow(sub_test_list)
    else:
        x_test,y_test,rgb_test,foot_ii_jj = getInputStack_saliency_rgb_mag_diff(sub_test_list)
    
    for ee in range(33):
        if os.path.exists('results/depth/{ee}.npy'.format(ee=ee)):
            model = tf.keras.models.load_model('models_{this_mode}/{ee}'.format(this_mode=mode,ee=ee))
            ...
            for idx,pred_map in enumerate(list(test)):
                ...
                try:
                    for edx in range(this_ii.shape[0]):
                        ...
                        this_val = pred_map[this_ii[edx],this_jj[edx]]
                        
                        
                        this_quant = np.sum(this_val>pred_map[include_dex])/(np.sum(include_dex))
    ...
    #            overlayed_img.append(out_img)
            ...
            print(str(ee)+'_'+str(np.nanmean(quants)))
            print(str(ee)+'_'+str(np.nanmedian(quants)))
            
        #    this_means.append(np.mean(quants))
        #    this_medians.append(np.median(quants))
        
            print(ee)
            np.save('results/{this_mode}/{ee}.npy'.format(this_mode=mode,ee=ee),{'mean':np.nanmean(quants),'median':np.nanmedian(quants)})
    #model.save('63')
